...
$stmt = $conn->prepare("SELECT id, title, content, image_url, created_at, flagged FROM posts WHERE user_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($post_id, $title, $content, $image_url, $created_at, $flagged);
    ...
    <div class="max-w-2xl mx-auto mt-10">
        ...
        <?php while ($stmt->fetch()): ?>
                <div class="border p-4 mb-4">
                    ...
                    <?php if ($flagged): ?>
                            <span class="bg-red-100 text-red-600 px-4 py-2 rounded">Flagged</span>
                    <?php else: ?>
                            <span class="bg-green-100 text-green-600 px-4 py-2 rounded">Not Flagged</span>
                    <?php endif; ?>
                    <a href="view_post.php?id=<?php echo $post_id; ?>" class="text-blue-500 block mt-2">View Comments</a>
                    <a href="delete_post.php?id=<?php echo $post_id; ?>" class="text-red-500 block mt-2">Delete Post</a>
                </div>
        <?php endwhile; ?>
    </div>
